...
###  1. Настройка NAT(PAT) на R14 и R15. Трансляция осуществляется в адрес AS1001:
###  Пример настройки NAT на роутере R14
```
! Создаем список доступа для определения внутренней сети
access-list 18 permit 192.168.96.0 0.0.15.255
! Создаем пул из публичных адресов
ip nat pool POOL-NAT 101.0.0.1 101.0.0.50 netmask 255.255.255.0
...
access-list 18 permit 192.168.96.0 0.0.15.255
! Создаем пул из публичных адресов
ip nat pool POOL-NAT 101.0.0.51 101.0.0.100 netmask 255.255.255.0
!Связываем ACL и пул, включая PAT
ip nat inside source list 18 pool POOL-NAT overload
